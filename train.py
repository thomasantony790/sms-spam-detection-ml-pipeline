...
def load_user_dataset():
    # If user data file does not exist, return empty dataframe
    if not os.path.exists(USER_DATA_PATH):
        return pd.DataFrame(columns=["label", "text"])

    # If file exists but is empty, return empty dataframe
    if os.path.getsize(USER_DATA_PATH) == 0:
        return pd.DataFrame(columns=["label", "text"])

    # Load user messages
    # Expected columns: label,text,source,created_at
    df = pd.read_csv(USER_DATA_PATH, encoding="utf-8")

    # If required columns are missing, ignore file
